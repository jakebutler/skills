...
| Live toolchain remediation: external pnpm and Vite scratch | The first production native arm could inspect the repository but the custom profile could not execute the attested Homebrew/Corepack `pnpm`; after enabling that toolchain, denying temporary writes prevented Vite from loading the focused suite. | v4 inherits execution capability from `:workspace`, overlays root denial plus explicit approved-toolchain reads, probe-verifies held-out/evidence/network/nested-agent denial, and explicitly attests writable temporary scratch because it is required by the visible test toolchain. The full proof suite and a live 13-test Vitest probe pass before the replacement arm launches. |
| Tracer 7 apparatus correction: oracle semantics and terminal remediation | A lexical proximity gate rejected valid constant indirection; both stock routes then omitted normal failed-check results and could not run the approved remediation from their dirty candidates or enter paired validation. | Native v5 and Composer v2 add content-bound held-out evaluator self-tests covering semantic positive/negative variants before launch. Both producers write terminal failed-check results and receipts, generation-1 configs authenticate the exact dirty parent state, and paired validation reports failed/remediated outcomes without treating them as scoreable success. |
| Fable exact-candidate remediation: leakage guards and invalid terminal rows | Architecture review found the new self-test field absent from two worker-packet leakage guards and found that `invalid-*` results emitted empty check arrays that the paired validator rejected. It also found that self-test vectors were not compared across remediation generations. | Native launch now rejects every unapproved worker-packet key, both finalizer and paired validator explicitly reject held-out self-test leakage, invalid results preserve approved check commands with `null` skipped statuses, and parent validation recursively authenticates receipts and exact check/self-test vectors. |
| Fable re-review remediation: timeout outcome ambiguity | Re-review found that Node reports a timeout-killed check with `status: null`, which the paired validator had reserved for deliberately skipped checks. | Every check row now records explicit `skipped` and `timed_out` booleans plus signal. Timed-out executed checks remain terminal failures that validate and may seed bounded remediation; invalidated pre-check rows remain distinguishable skipped outcomes. |
| Sol exact-candidate remediation: non-timeout signal outcome | The fallback final gate reproduced a check self-terminated by `SIGTERM` as `status: null`, `signal: SIGTERM`, and `timed_out: false`; the validator still required an integer status. | Signaled checks are a distinct validator-readable failure outcome, fixtures cover producer and paired classification, and the result schema uses a four-way `oneOf` for normally exited, signaled, timed-out, and deliberately skipped rows. |
...
`native-sol-exec-v5-rehearsal-2026-07-20.json` records the post-Tracer-7 producer
rehearsal. Native Sol v5 and Cursor Composer v2 completed the same disposable task,
produced equal working state, passed the capability probe and paired validation, and
stored 125 hash-manifested evidence files in the durable evidence directory. This is
apparatus-only evidence, not a model-quality result.
