...
9. Orchestrator: decide whether the blast radius requires broader verification. Medium and High tiers: independent verification recorded in verification.md, or a documented reason why not.
10. `verifier`: when independent verification runs, execute scoped checks and affected-flow verification without editing code and write results to `{{TASK_DOCS_DIR}}/{{TASK_ID}}/verification.md`.
