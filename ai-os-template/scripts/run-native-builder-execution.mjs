...
export async function runNativeBuilderExecution(
  ...
  if ([transcriptPath, stderrPath, completionPath, attestationPath, executionClaimPath, packet.result_path, packet.finalize_receipt_path].some((target) => fs.existsSync(target))) {
  ...
  const executable = testOverrides.codexExecutable ?? "codex";
  const executablePath = resolveNativeCodexExecutable(resolveExecutable(executable, environment));
